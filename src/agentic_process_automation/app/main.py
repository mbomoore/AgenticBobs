...
# Try to import core functionality with fallbacks
try:
    from ..core.adapters.bpmn import parse_bpmn
except ImportError:
    try:
        from ..core.adapters.bpmn_min import from_bpmn_xml
        def parse_bpmn(xml_bytes):
            return from_bpmn_xml(xml_bytes)
    except ImportError:
        def parse_bpmn(xml_bytes):
            raise ImportError("No BPMN parser available")

from ..core.pir import validate
from ..core.viz import pir_to_mermaid

# Try to import AI functionality, fall back gracefully
try:
    from ..core.ai import (
        chat as ai_chat, extract_bpmn_xml, system_prompt, 
        agent_chat, agent_system_prompt
    )
    AI_AVAILABLE = True
except ImportError:
    AI_AVAILABLE = False
    
# Try to import CLI tools for alternative approach
try:
        ...
        if user_msg:
                ...
                with st.chat_message("assistant"):
                        ...
                        with st.spinner(spinner_text):
                                if not AI_AVAILABLE:
                                        st.error("AI functionality not available. Please install the 'ai' dependencies.")
                                        st.session_state.chat_messages.append({"role": "assistant", "content": "AI functionality not available. Please install optional dependencies for full features."})
                                else:
                                        try:
...
def main():
    ...
    st.markdown("*An AI agent for process formalization and automation*")
    
    if not AI_AVAILABLE and not CLI_AVAILABLE:
        st.warning("⚠️ AI functionality not available. Install optional dependencies for full features.")
    
    # The main app logic is in the global scope above
    # This structure follows the original Streamlit pattern
